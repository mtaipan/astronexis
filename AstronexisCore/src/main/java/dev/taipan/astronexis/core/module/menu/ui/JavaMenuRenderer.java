...
public final class JavaMenuRenderer {
    ...
    public JavaMenuRenderer(AstronexisCorePlugin plugin, MessageService messages) {
        this.plugin = plugin;
    ...
    public void open(Player player, MenuPage page) {
        String title = switch (page) {
            case MAIN -> messages.get(player, "menu.title-main-java");
            case HELP -> messages.get(player, "menu.title-help-java");
        ...
        Inventory inv = Bukkit.createInventory(new MenuHolder(page, false), 27, title);
        fill(inv);
    ...
    private void buildMain(Player player, Inventory inv) {
        inv.setItem(11, item(
                ...
                List.of(
                        messages.get(player, "menu.item-survival-lore-1"),
                        messages.get(player, "menu.item-survival-lore-2")
                )
        ));
